...
class StayController
{
        ...
        public function store($data)
    {
        ...
        $reservation_date = array_key_exists('reservation_date', $data) && $data['reservation_date'] ? $data['reservation_date'] : '';
        $start_date = array_key_exists('start_date', $data) && $data['start_date'] ? $data['start_date'] : '';
        $end_date = array_key_exists('end_date', $data) && $data['end_date'] ? $data['end_date'] : '';
        $animal_id = array_key_exists('animal_id', $data) && $data['animal_id'] ? $data['animal_id'] : '';

        if ($data && $reservation_date && $start_date && $end_date && $animal_id && $validationResult === "") {
    ...
    private function validateStayData($data)
    {
        ...
        if ($reservationDate > $startDate) {
            return 'La date de réservation doit être avant ou le même jour que la date de début.'; //pose problème pour l'update future
        }
